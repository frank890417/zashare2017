    ...
    el-tabs.mt-5.mb-3(v-model="activeName" @tab-click="handleClick")
      el-tab-pane(label="報名紀錄" name="regist" ) 報名紀錄
        ...
        el-table(:data="filteredRegistexpo" border)
          ...
          el-table-column(prop="status",label="狀態", width="120" :sortable="true")
            template(slot-scope="scope")
              span {{ getStatus(scope.row.status).label }}
          el-table-column(prop="paid_record_status",label="繳款狀態", width="120" :sortable="true")
          //- el-table-column(prop="tag",label="類別", width="80" :sortable="true")
          el-table-column(prop="name_cht",label="中文", width="150" :sortable="true")
          el-table-column(prop="name_eng",label="英文", width="150" :sortable="true")
          el-table-column(prop="type",label="單位種類", width="250" :sortable="true")
          el-table-column(prop="theme",label="類別", width="120" :sortable="true")

          //- el-table-column(prop="description",label="描述", width="200" :sortable="true")
          //- el-table-column(prop="target_audience",label="目標受眾", width="200" :sortable="true")
          //- el-table-column(prop="want_audience",label="希望觸及受眾", width="200" :sortable="true")
          //el-table-column(prop="have_sell",label="有銷售", width="100" :sortable="true")
            template(slot-scope="scope")
              span {{scope.have_sell?"有":"無"}}
          //el-table-column(prop="attend_reason",label="參與原因", width="200" :sortable="true")
          ...
          el-table-column(prop="main_contact_name",label="主要聯絡人", width="150" :sortable="true")
          el-table-column(prop="main_contact_email",label="主要信箱", width="200" :sortable="true")
          el-table-column(prop="secondary_contact_name",label="次要聯絡人", width="150" :sortable="true")
      ...
      el-tab-pane(label="繳費記錄" name="paidrecord") 繳費記錄
        ...
        el-table(:data="paidRecords" border)
          ...
          el-table-column(prop="status",label="狀態", width="120" :sortable="true")
            template(slot-scope="scope")
              span {{ getStatus(scope.row.status).label }}
          el-table-column(prop="registname",label="攤位名稱", width="200" :sortable="true")
          el-table-column(prop="paid_datetime",label="確認繳費", width="160" :sortable="true")
            template(slot-scope="scope")
              div
          ...
          el-table-column(prop="paid_amount",label="繳費金額", width="200" :sortable="true")
          el-table-column(prop="paid_datetime",label="繳費時間", width="200" :sortable="true")
          el-table-column(prop="paid_direct",label="臨櫃匯款", width="120" )
            template(slot-scope ="scope")
              span {{ scope.row.paid_direct?'是':'否' }}
          el-table-column(prop="paid_name",label="名字", width="100" :sortable="true")
          el-table-column(prop="paid_last_number",label="後五碼", width="100" :sortable="true")
        ...
        el-table(:data="workshops" border)
          ...
          el-table-column(prop="status",label="狀態", width="120" :sortable="true")
            template(slot-scope="scope")
              span {{ getStatus(scope.row.status).label }}
          el-table-column(prop="registname",label="攤位名稱", width="200" :sortable="true")
          el-table-column(prop="class_type",label="課程類型", width="200" :sortable="true")
          el-table-column(prop="audience_normal",label="活動招生族群", width="160" )
            template(slot-scope ="scope")
              span {{ scope.row.audience_normal || '無限制' }}
          el-table-column(prop="class_person_count",label="預計人數", width="130" :sortable="true")
          el-table-column(prop="class_proposal",label="提案簡報", width="80")
            template(slot-scope="scope")
              a(:href="apiDomain+scope.row.class_proposal.replace('/storage/app/public','storage')", target="_href") 連結
          //- el-table-column(prop="agree_plan",label="同意規劃", width="200" :sortable="true")
          el-table-column(prop="main_contact_name",label="主要聯絡人", width="150" :sortable="true")
          el-table-column(prop="main_contact_email",label="主要信箱", width="200" :sortable="true")
          el-table-column(prop="secondary_contact_name",label="次要聯絡人", width="150" :sortable="true")
          el-table-column(prop="secondary_contact_email",label="次要信箱", width="200" :sortable="true")
        ...
        el-table(:data="registSpeaks" border)
          ...
          el-table-column(prop="status",label="狀態", width="120" :sortable="true")
            template(slot-scope="scope")
              span {{ getStatus(scope.row.status).label }}
          el-table-column(prop="registname",label="攤位名稱", width="200" :sortable="true")
          el-table-column(prop="agree_plan",label="同意策展權益", width="130" :sortable="true")
            template(slot-scope ="scope")
              span {{ scope.row.agree_plan?'是':'否' }}
          el-table-column(prop="startup_content",label="創業內容", width="200" :sortable="true")
          el-table-column(prop="startup_problem",label="想解決的問題", width="200" :sortable="true")
          el-table-column(prop="startup_proposal",label="提案簡報", width="80")
            template(slot-scope="scope")
              a(:href="apiDomain+scope.row.startup_proposal.replace('/storage/app/public','storage')", target="_href") 連結
      ...
      //- el-table-column(prop="year",label="年份" width="60" :sortable="true")
